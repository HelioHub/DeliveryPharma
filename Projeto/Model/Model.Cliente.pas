...
begin
  ...
  try
    ...
    Result := True; // Indica que o cliente foi excluído com sucesso
  except
    on E: Exception do
    begin
...
begin
  ...
  try
    ...
    try
      // Prepara a query para inserir ou atualizar o cliente
      FQuery.SQL.Clear;
      if FCodigoClientes = 0 then
      begin
        // Inserir novo cliente
        FQuery.SQL.Text := 'INSERT INTO Clientes (CodigoClientes, CNPJClientes, NomeClientes, CEPClientes, RuaClientes, ' +
                           ...
                           'VALUES (GEN_ID(GEN_Clientes_ID, 1), :CNPJ, :Nome, :CEP, :Rua, :Bairro, :Cidade, :UF, :Longitude, :Latitude, ' +
      ...
      begin
        FQuery.SQL.Clear;
        FQuery.SQL.Add('SELECT GEN_ID(GEN_Clientes_ID, 0) AS LastID FROM RDB$DATABASE ');
        FQuery.Open;
        FCodigoClientes := FQuery.FieldByName('LastID').AsInteger;
      end;

      FDatabaseConnection.Connection.Commit;
      Result := True; // Indica que o cliente foi salvo com sucesso
    except
      on E: Exception do
      begin
